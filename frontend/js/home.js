/* ============================================================
   R BD SHOP — Home Page JavaScript
   Path: frontend/js/home.js
   Description: Main entry point for home page. Loads header,
                categories, featured products, reviews, settings,
                stats counter, theme, lazy load, back-to-top.
   ============================================================ */

import { db }          from './firebase-config.js';
import {
  collection, query, where, orderBy,
  limit, getDocs, doc, getDoc, onSnapshot
}                      from 'https://www.gstatic.com/firebasejs/10.12.2/firebase-firestore.js';

import { initTheme }   from './theme.js';
import { buildHeaderHTML, initHeader } from './header.js';
import {
  hidePageLoader, initBackToTop, initLazyLoad,
  formatPrice, escapeHtml, renderProductCard,
  renderStars, renderProductSkeletons,
  getCategoryUrl, showToast
}                      from './utils.js';

/* ─────────────────────────────────────────────────────────────
   CONSTANTS
───────────────────────────────────────────────────────────── */
const PRODUCTS_PER_PAGE = 8;
let allProducts         = [];
let displayedCount      = 0;
let currentFilter       = 'all';
let shopSettings        = {};

/* ─────────────────────────────────────────────────────────────
   INITIALIZATION
───────────────────────────────────────────────────────────── */
document.addEventListener('DOMContentLoaded', async () => {
  // 1. Theme init (FOUC prevention already done in <head>)
  initTheme();

  // 2. Load shop settings from Firestore
  await loadSettings();

  // 3. Inject header HTML and init
  injectHeader();
  initHeader();

  // 4. Load all data in parallel
  await Promise.allSettled([
    loadCategories(),
    loadFeaturedProducts(),
    loadReviews(),
    loadStats(),
  ]);

  // 5. Lazy load images
  initLazyLoad();

  // 6. Back to top button
  initBackToTop();

  // 7. Footer dynamic data
  updateFooter();

  // 8. Footer year
  const yearEl = document.getElementById('footer-year');
  if (yearEl) yearEl.textContent = new Date().getFullYear();

  // 9. Hide page loader
  hidePageLoader();
});

/* ─────────────────────────────────────────────────────────────
   SETTINGS — Firestore থেকে shop settings load করে
───────────────────────────────────────────────────────────── */
async function loadSettings() {
  try {
    const settingsRef = doc(db, 'settings', 'shopSettings');
    const snap        = await getDoc(settingsRef);
    if (snap.exists()) {
      shopSettings = snap.data();
    }
  } catch (err) {
    console.warn('[R BD SHOP] Settings load failed:', err);
  }
}

/* ─────────────────────────────────────────────────────────────
   HEADER INJECTION
───────────────────────────────────────────────────────────── */
function injectHeader() {
  const headerRoot = document.getElementById('header-root');
  if (!headerRoot) return;
  headerRoot.innerHTML = buildHeaderHTML(shopSettings);
}

/* ─────────────────────────────────────────────────────────────
   CATEGORIES — Firestore থেকে load করে grid-এ render করে
───────────────────────────────────────────────────────────── */
async function loadCategories() {
  const grid = document.getElementById('categories-grid');
  if (!grid) return;

  try {
    const catQuery = query(
      collection(db, 'categories'),
      where('status', '==', 'active'),
      orderBy('name'),
      limit(12)
    );

    const snapshot = await getDocs(catQuery);

    if (snapshot.empty) {
      grid.innerHTML = `
        <div class="empty-state" style="grid-column:1/-1;padding:var(--space-10)">
          <p class="text-muted">এখনো কোনো ক্যাটাগরি যোগ করা হয়নি।</p>
        </div>`;
      return;
    }

    let html = '';
    const filterTabsContainer = document.getElementById('filter-tabs');

    snapshot.forEach((docSnap) => {
      const cat = docSnap.data();
      const catUrl = getCategoryUrl(cat.slug || cat.name);

      html += `
        <a href="${catUrl}" class="category-card animate-fade-up">
          <div class="category-card__icon">
            ${cat.imageURL
              ? `<img src="${escapeHtml(cat.imageURL)}" alt="${escapeHtml(cat.name)}" loading="lazy" />`
              : `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                   <rect x="3" y="3" width="7" height="7"/>
                   <rect x="14" y="3" width="7" height="7"/>
                   <rect x="14" y="14" width="7" height="7"/>
                   <rect x="3" y="14" width="7" height="7"/>
                 </svg>`
            }
          </div>
          <span class="category-card__name">${escapeHtml(cat.name)}</span>
          ${cat.productCount ? `<span class="category-card__count">${cat.productCount} পণ্য</span>` : ''}
        </a>`;

      // Filter tab যোগ করো
      if (filterTabsContainer) {
        const tabBtn = document.createElement('button');
        tabBtn.className = 'filter-tab';
        tabBtn.dataset.filter = cat.name;
        tabBtn.textContent = cat.name;
        tabBtn.addEventListener('click', () => filterProducts(cat.name, tabBtn));
        filterTabsContainer.appendChild(tabBtn);
      }
    });

    grid.innerHTML = html;

  } catch (err) {
    console.error('[R BD SHOP] Categories load error:', err);
    grid.innerHTML = `
      <div class="empty-state" style="grid-column:1/-1;padding:var(--space-10)">
        <p class="text-muted">ক্যাটাগরি লোড করতে সমস্যা হয়েছে।</p>
      </div>`;
  }
}

/* ─────────────────────────────────────────────────────────────
   FEATURED PRODUCTS — Firestore থেকে published products load
───────────────────────────────────────────────────────────── */
async function loadFeaturedProducts() {
  const grid = document.getElementById('products-grid');
  if (!grid) return;

  // Show skeletons while loading
  grid.innerHTML = renderProductSkeletons(PRODUCTS_PER_PAGE);

  try {
    const prodQuery = query(
      collection(db, 'products'),
      where('status', '==', 'published'),
      orderBy('createdAt', 'desc'),
      limit(50) // max 50 for home page
    );

    const snapshot = await getDocs(prodQuery);

    if (snapshot.empty) {
      grid.innerHTML = `
        <div class="empty-state" style="grid-column:1/-1;padding:var(--space-16)">
          <div class="empty-state__icon">
            <svg width="40" height="40" viewBox="0 0 24 24" fill="none"
                 stroke="currentColor" stroke-width="1.5">
              <path d="M6 2L3 6v14a2 2 0 002 2h14a2 2 0 002-2V6l-3-4z"/>
              <line x1="3" y1="6" x2="21" y2="6"/>
              <path d="M16 10a4 4 0 01-8 0"/>
            </svg>
          </div>
          <h3 class="empty-state__title">কোনো পণ্য নেই</h3>
          <p class="empty-state__desc">এখনো কোনো পণ্য যোগ করা হয়নি। শীঘ্রই নতুন পণ্য আসবে!</p>
        </div>`;
      return;
    }

    allProducts = [];
    snapshot.forEach((docSnap) => {
      allProducts.push({ id: docSnap.id, ...docSnap.data() });
    });

    // Update hero stat
    const statProductsEl = document.getElementById('stat-products');
    if (statProductsEl) statProductsEl.textContent = `${allProducts.length}+`;

    // Render first batch
    displayedCount = 0;
    renderProducts();

  } catch (err) {
    console.error('[R BD SHOP] Products load error:', err);
    grid.innerHTML = `
      <div class="empty-state" style="grid-column:1/-1;padding:var(--space-16)">
        <div class="empty-state__icon">
          <svg width="40" height="40" viewBox="0 0 24 24" fill="none"
               stroke="currentColor" stroke-width="1.5">
            <circle cx="12" cy="12" r="10"/>
            <path d="M15 9l-6 6M9 9l6 6"/>
          </svg>
        </div>
        <h3 class="empty-state__title">লোড করতে সমস্যা</h3>
        <p class="empty-state__desc">পণ্য লোড করতে সমস্যা হয়েছে। পেজ রিফ্রেশ করুন।</p>
      </div>`;
  }
}

/* ─────────────────────────────────────────────────────────────
   RENDER PRODUCTS (pagination + filter)
───────────────────────────────────────────────────────────── */
function renderProducts() {
  const grid         = document.getElementById('products-grid');
  const loadMoreWrap = document.getElementById('load-more-wrap');
  if (!grid) return;

  // Filter
  let filtered = allProducts;
  if (currentFilter !== 'all') {
    filtered = allProducts.filter((p) => p.categoryName === currentFilter);
  }

  // Slice for pagination
  const toShow  = filtered.slice(0, displayedCount + PRODUCTS_PER_PAGE);
  displayedCount = toShow.length;

  if (toShow.length === 0) {
    grid.innerHTML = `
      <div class="empty-state" style="grid-column:1/-1;padding:var(--space-10)">
        <h3 class="empty-state__title">এই ক্যাটাগরিতে কোনো পণ্য নেই</h3>
        <p class="empty-state__desc">অন্য ক্যাটাগরি দেখুন</p>
      </div>`;
    if (loadMoreWrap) loadMoreWrap.style.display = 'none';
    return;
  }

  // Render cards
  grid.innerHTML = toShow.map((p) => renderProductCard(p)).join('');

  // Lazy load new images
  initLazyLoad();

  // Show/hide load more
  if (loadMoreWrap) {
    loadMoreWrap.style.display = displayedCount < filtered.length ? 'block' : 'none';
  }
}

/**
 * Filter button click handler
 */
function filterProducts(categoryName, clickedBtn) {
  currentFilter  = categoryName;
  displayedCount = 0;

  // Active tab UI update
  const allTabs = document.querySelectorAll('.filter-tab');
  allTabs.forEach((t) => t.classList.remove('active'));
  clickedBtn.classList.add('active');

  renderProducts();
}

// "সব পণ্য" tab click
document.addEventListener('click', (e) => {
  if (e.target.matches('[data-filter="all"]')) {
    currentFilter  = 'all';
    displayedCount = 0;
    const allTabs = document.querySelectorAll('.filter-tab');
    allTabs.forEach((t) => t.classList.remove('active'));
    e.target.classList.add('active');
    renderProducts();
  }
});

// Load More button
document.addEventListener('click', (e) => {
  if (e.target.closest('#load-more-btn')) {
    renderProducts();
  }
});

/* ─────────────────────────────────────────────────────────────
   REVIEWS — Firestore থেকে approved reviews load করে
───────────────────────────────────────────────────────────── */
async function loadReviews() {
  const grid = document.getElementById('reviews-grid');
  if (!grid) return;

  try {
    const revQuery = query(
      collection(db, 'reviews'),
      where('status', '==', 'approved'),
      orderBy('createdAt', 'desc'),
      limit(6)
    );

    const snapshot = await getDocs(revQuery);

    if (snapshot.empty) {
      grid.innerHTML = `
        <div class="empty-state" style="grid-column:1/-1;padding:var(--space-8)">
          <p class="text-muted">এখনো কোনো রিভিউ নেই।</p>
        </div>`;
      return;
    }

    let html = '';
    snapshot.forEach((docSnap) => {
      const rev = docSnap.data();
      const initial = (rev.customerName || 'U').charAt(0).toUpperCase();

      html += `
        <div class="review-card animate-fade-up">
          <div class="review-card__top">
            <div style="display:flex;align-items:center;gap:var(--space-3)">
              <div class="review-card__avatar">${initial}</div>
              <div>
                <div class="review-card__name">${escapeHtml(rev.customerName || 'গ্রাহক')}</div>
                <div class="review-card__date">${rev.createdAt ? formatRelativeDate(rev.createdAt) : ''}</div>
              </div>
            </div>
            <div>${renderStars(rev.rating || 5)}</div>
          </div>
          ${rev.productName ? `<span class="review-card__product">📦 ${escapeHtml(rev.productName)}</span>` : ''}
          <p class="review-card__text">${escapeHtml(rev.comment || '')}</p>
        </div>`;
    });

    grid.innerHTML = html;

  } catch (err) {
    console.warn('[R BD SHOP] Reviews load error:', err);
    grid.innerHTML = `
      <div class="empty-state" style="grid-column:1/-1;padding:var(--space-8)">
        <p class="text-muted">রিভিউ লোড করতে সমস্যা হয়েছে।</p>
      </div>`;
  }
}

/**
 * Relative date helper (for reviews)
 */
function formatRelativeDate(timestamp) {
  if (!timestamp) return '';
  let d;
  if (typeof timestamp.toDate === 'function') {
    d = timestamp.toDate();
  } else {
    d = new Date(timestamp);
  }
  if (isNaN(d.getTime())) return '';

  const now  = Date.now();
  const diff = now - d.getTime();
  const mins = Math.floor(diff / 60000);
  const hrs  = Math.floor(mins / 60);
  const days = Math.floor(hrs / 24);

  if (mins < 1)   return 'এইমাত্র';
  if (mins < 60)  return `${mins} মিনিট আগে`;
  if (hrs < 24)   return `${hrs} ঘণ্টা আগে`;
  if (days < 7)   return `${days} দিন আগে`;
  if (days < 30)  return `${Math.floor(days / 7)} সপ্তাহ আগে`;
  return d.toLocaleDateString('bn-BD', { year:'numeric', month:'short', day:'numeric' });
}

/* ─────────────────────────────────────────────────────────────
   STATS — Firestore-এ থেকে dynamic stats load করে
───────────────────────────────────────────────────────────── */
async function loadStats() {
  try {
    // Total products count (already loaded in allProducts if available)
    const totalEl = document.getElementById('stat-total-products');
    if (totalEl && allProducts.length > 0) {
      totalEl.textContent = `${allProducts.length}+`;
    } else if (totalEl) {
      const prodSnap = await getDocs(
        query(collection(db, 'products'), where('status', '==', 'published'))
      );
      totalEl.textContent = `${prodSnap.size}+`;
    }

    // Customers count
    const custEl = document.getElementById('stat-customers');
    if (custEl) {
      try {
        const custSnap = await getDocs(collection(db, 'customers'));
        const count    = custSnap.size;
        custEl.textContent = count > 0 ? `${count}+` : '১০০+';
      } catch { /* default stays */ }
    }

    // Deliveries count (completed orders)
    const delEl = document.getElementById('stat-deliveries');
    if (delEl) {
      try {
        const orderSnap = await getDocs(
          query(collection(db, 'orders'), where('status', '==', 'delivered'))
        );
        const count = orderSnap.size;
        delEl.textContent = count > 0 ? `${count}+` : '৫০০+';
      } catch { /* default stays */ }
    }

  } catch (err) {
    console.warn('[R BD SHOP] Stats load error:', err);
  }
}

/* ─────────────────────────────────────────────────────────────
   FOOTER — Dynamic links & social icons update
───────────────────────────────────────────────────────────── */
function updateFooter() {
  const s = shopSettings;

  // WhatsApp support links
  if (s.whatsappNumber) {
    const waNum = s.whatsappNumber.replace(/\D/g, '');
    const waUrl = `https://wa.me/${waNum}`;

    const supportWA = document.getElementById('support-whatsapp');
    if (supportWA) supportWA.href = waUrl;

    const footerWA = document.getElementById('footer-wa');
    if (footerWA) footerWA.href = waUrl;
  }

  // Telegram links
  if (s.telegramLink || s.telegramUsername) {
    const tgUrl = s.telegramLink || `https://t.me/${(s.telegramUsername || '').replace('@', '')}`;

    const supportTG = document.getElementById('support-telegram');
    if (supportTG) supportTG.href = tgUrl;

    const footerTG = document.getElementById('footer-tg');
    if (footerTG) footerTG.href = tgUrl;
  }

  // Social icons in footer
  const socialContainer = document.getElementById('footer-social');
  if (socialContainer) {
    let socialHtml = '';

    if (s.facebookLink) {
      socialHtml += `
        <a href="${escapeHtml(s.facebookLink)}" class="footer__social-link"
           target="_blank" rel="noopener" aria-label="Facebook" title="Facebook">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
            <path d="M18 2h-3a5 5 0 00-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 011-1h3z"/>
          </svg>
        </a>`;
    }

    if (s.youtubeLink) {
      socialHtml += `
        <a href="${escapeHtml(s.youtubeLink)}" class="footer__social-link"
           target="_blank" rel="noopener" aria-label="YouTube" title="YouTube">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
            <path d="M22.54 6.42a2.78 2.78 0 00-1.94-2C18.88 4 12 4 12 4s-6.88 0-8.6.46
                     a2.78 2.78 0 00-1.94 2A29 29 0 001 11.75a29 29 0 00.46 5.33A2.78 2.78
                     0 003.4 19.1c1.72.46 8.6.46 8.6.46s6.88 0 8.6-.46a2.78 2.78 0 001.94-2
                     29 29 0 00.46-5.25 29 29 0 00-.46-5.43z"/>
            <polygon points="9.75 15.02 15.5 11.75 9.75 8.48 9.75 15.02"/>
          </svg>
        </a>`;
    }

    if (s.whatsappNumber) {
      const waNum = s.whatsappNumber.replace(/\D/g, '');
      socialHtml += `
        <a href="https://wa.me/${waNum}" class="footer__social-link"
           target="_blank" rel="noopener" aria-label="WhatsApp" title="WhatsApp">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
            <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15
                     -.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463
                     -2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606
                     .134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371
                     -.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51
                     -.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372s-1.04 1.016-1.04
                     2.479 1.065 2.875 1.213 3.074c.149.198 2.096 3.2 5.077 4.487.709.306
                     1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719
                     2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/>
          </svg>
        </a>`;
    }

    if (s.telegramLink || s.telegramUsername) {
      const tgUrl = s.telegramLink || `https://t.me/${(s.telegramUsername || '').replace('@', '')}`;
      socialHtml += `
        <a href="${escapeHtml(tgUrl)}" class="footer__social-link"
           target="_blank" rel="noopener" aria-label="Telegram" title="Telegram">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
            <path d="M11.944 0A12 12 0 000 12a12 12 0 0012 12 12 12 0 0012-12A12 12 0
                     0012 0h-.056zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0
                     01.171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627
                     -.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693
                     -1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247
                     -2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c
                     -.106.024-1.793 1.14-5.061 3.345-.479.33-.913.49-1.302.48-.428-.008
                     -1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325
                     -.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386
                     4.025-1.627 4.476-1.635z"/>
          </svg>
        </a>`;
    }

    socialContainer.innerHTML = socialHtml || '<span class="text-muted text-sm">কোনো সোশ্যাল লিংক নেই</span>';
  }
  }
